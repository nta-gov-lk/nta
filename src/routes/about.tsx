  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "About the National Television Academy" },
      {
        ...
        content: "Our history, SLRC affiliation, vision and mission, and the broadcast facilities students train in.",
      },
...
const leadership = [
  { role: "Director of the Academy", name: "[PLACEHOLDER — name]", note: "Academic direction and programme oversight." },
  { role: "Head of Training", name: "[PLACEHOLDER — name]", note: "Curriculum, instructors and studio scheduling." },
  { role: "Studio Operations Lead", name: "[PLACEHOLDER — name]", note: "Facilities, equipment and production support." },
];
    ...
    <>
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            ...
            <p className="mt-6 text-sm leading-relaxed text-muted-foreground">
              Students learn where programmes are actually made. Instructors are working professionals — directors,
              presenters, camera operators, lighting technicians and dubbing artists — who teach the standards they are
              held to on transmission days. That proximity to real production is the academy's defining advantage.
            </p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <img src={studioA} alt="Presenting practice at the studio news desk" loading="lazy" className="h-72 w-full rounded-xl object-cover" />
            <img src={studioB} alt="Camera and lighting training session" loading="lazy" className="mt-0 h-72 w-full rounded-xl object-cover sm:mt-8" />
          </div>
        </div>
      </Section>
      ...
      <Section className="bg-card">
        <div className="grid gap-6 md:grid-cols-2">
          {[
            { icon: Eye, title: "Our Vision", body: "To be Sri Lanka's leading centre for practical television and media education, producing graduates who raise the standard of national broadcasting." },
            { icon: Target, title: "Our Mission", body: "To deliver hands-on, industry-led training in performance, presenting and broadcast technology in all three national languages, inside genuine production facilities." },
          ].map((c) => (
        ...
        <div className="mt-10 space-y-6 border-l-2 border-gold pl-6">
          {timeline.map((t) => (
            <div key={t.year + t.title} className="relative">
              <span className="absolute -left-[31px] top-1.5 h-4 w-4 rounded-full border-2 border-gold bg-background" aria-hidden />
              <p className="font-display text-sm font-bold uppercase tracking-widest text-primary">{t.year}</p>
      ...
      <Section className="bg-card">
        <SectionHeading eyebrow="Leadership" title="Who leads the academy" intro="Names and titles below are placeholders pending official confirmation." />
      ...
      <Section>
        ...
        <ul className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {facilities.map((f) => (
            <li key={f} className="flex items-start gap-3 rounded-lg border border-border bg-card p-5">
